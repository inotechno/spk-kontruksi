...
class breadcrumb
{
 ...
 public function show($home_site ="Home", $id = "crumbs"  ) 
 {
  ...
  $out  = '<ol class="breadcrumb float-sm-right" id="'.$id.'">'; // <ul class="breadcrumb">
  if ($breadcrumbs && count($breadcrumbs) > 0) {
   $out .= '<li class="breadcrumb-item"><a href="' . base_url() .'"/>'. $site . '</a></li>'; // <li><a href="#">Home</a></li>
   $i=1;
   foreach ($breadcrumbs as $crumb): 
    ...
    if ($i != count($breadcrumbs)) {
     ...
     $out .= '<li class="breadcrumb-item active">'. $crumb['title'] .'</li>';
    }
    $i++;
   endforeach;
  } else {
   $out .= '<li class="breadcrumb-item active">' . $site . '</li>';
  }
  $out .= '</ol>';
  return $out; 
 }
